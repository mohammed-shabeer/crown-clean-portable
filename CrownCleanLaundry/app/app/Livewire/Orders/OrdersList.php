...
class OrdersList extends Component
{
    ...
    public $hasMorePages;
    public $paid_filter;
    public $orderToDelete = null; // For delete confirmation
    ...
    public function updated($name, $value)
    {
        ...
        // $this->reloadOrders();
        // All users can see all orders (removed created_by filter for non-admin users)
        $ordersQuery =  \App\Models\Order::withSum('payments', 'received_amount')->orderBy('order_number','DESC');
        ...
        if ($name == 'search_query') {
            ...
            if ($this->paid_filter == '') {
                $this->orders = $ordersQuery->limit(50)->get();
            } elseif ($this->paid_filter != '') {
                $paymentStatus = $this->paid_filter;
                // Use eager-loaded payment sums instead of N+1 queries
                $this->orders = $this->filterByPaymentStatus($ordersQuery->limit(50)->get(), $paymentStatus);
            }
        }
        ...
        if ($name == 'paid_filter') {
            if ($value != '') {
                ...
                $this->orders = $ordersQuery->orderBy('order_number','DESC')->limit(50)->get();
            }
        }
    }
    ...
    /**
     * Filter orders by payment status using eager-loaded payment sums.
     * Uses withSum instead of N+1 queries.
     */
    protected function filterByPaymentStatus($orders, $paymentStatus)
    {
        return $orders->filter(function ($order) use ($paymentStatus) {
    ...
    public function filterdata()
    {
        if ($this->search_query || $this->search_query != '') {
            if ($this->order_filter || $this->order_filter != '') {
                // All users can see all orders - with eager-loaded payment sums
                $orders = \App\Models\Order::withSum('payments', 'received_amount')
    ...
    public function confirmDelete($orderId)
    {
        if(!\Illuminate\Support\Facades\Gate::allows('order_delete')){
            $this->dispatch(
                'alert',
                ['type' => 'error',  'message' => 'You do not have permission to delete orders!']
            );
            return;
        }
    ...
    public function deleteOrder($order = null)
    {
        if(!\Illuminate\Support\Facades\Gate::allows('order_delete')){
            $this->dispatch(
                'alert',
        ...
        $orderId = $order ?? $this->orderToDelete;
