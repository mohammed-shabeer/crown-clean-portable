<div class="dashboard-main-body">
        ...
        <div class="tw-py-1.5 tw-px-3 bg-base d-flex align-items-center flex-wrap gap-3 justify-content-between">
            <div class="d-flex align-items-center flex-wrap gap-3">
                <form class="navbar-search">
                    <input type="text" class="bg-base tw-px-3 tw-py-1.5 w-auto" name="search" placeholder="{{ $lang->data['search_here'] ?? 'Search Here' }}" wire:model.live.debounce.300ms="search_query">
                        ...
                        <tr class="tw-text-xs">
                            <td>
                                <div class="tw-flex tw-flex-col">
                                    ...
                                    @if(isset($item->order_type) && $item->order_type == 2)
                                    <div class="tw-mt-1">
                                        <span class="badge fw-semibold bg-purple-100 text-purple-600 px-8 py-4 radius-4 tw-text-[10px]">
                                            <iconify-icon icon="mdi:qrcode" class="tw-mr-0.5"></iconify-icon>
                                            {{ $lang->data['online_order'] ?? 'Online Order' }}
                                        </span>
                                    </div>
                                    @endif
                                </div>
                            ...
                            <td class="">
                                @if(isset($item->order_type) && $item->order_type == 2)
                                    <span class="tw-text-purple-600 tw-font-medium">
                                        <iconify-icon icon="mdi:account-outline" class="tw-mr-0.5"></iconify-icon>
                                        {{ $lang->data['customer_self_order'] ?? 'Customer' }}
                                    </span>
                                @else
                                    {{ $item->user->name ?? "" }}
                                @endif
                            ...
                            <td class="text-center"> 
                                <div class="d-flex align-items-center gap-10 justify-content-center">
                                    ...
                                    @endcan
                                    @can('order_delete')
                                    <button type="button" wire:click.prevent="confirmDelete({{$item->id}})" data-bs-toggle="modal" data-bs-target="#deleteConfirmModal" class="remove-item-button bg-danger-focus bg-hover-danger-200 text-danger-600 fw-medium tw-size-8 d-flex justify-content-center align-items-center rounded-circle"> 
    ...
    <div class="modal fade" id="deleteConfirmModal" tabindex="-1" aria-labelledby="deleteConfirmModalLabel" aria-hidden="true" wire:ignore.self>
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content radius-16 bg-base">
                <div class="modal-header py-16 px-24 border border-top-0 border-start-0 border-end-0">
                    <h1 class="modal-title text-md" id="deleteConfirmModalLabel">{{ $lang->data['confirm_delete'] ?? 'Confirm Delete' }}</h1>
                ...
                <div class="modal-body p-24 text-center">
                    <div class="mb-20 d-flex justify-content-center">
                        <iconify-icon icon="fluent:warning-24-filled" class="text-warning-600" style="font-size: 64px;"></iconify-icon>
                    </div>
                    <h5 class="mb-12">{{ $lang->data['are_you_sure'] ?? 'Are you sure?' }}</h5>
                    <p class="text-secondary-light">{{ $lang->data['delete_order_confirm'] ?? 'Do you really want to delete this order? This action cannot be undone.' }}</p>
                </div>
                <div class="modal-footer d-flex align-items-center justify-content-center gap-3 py-16 px-24">
                    <button type="button" class="border border-neutral-400 bg-hover-neutral-200 text-neutral-600 text-md px-40 py-11 radius-8" data-bs-dismiss="modal" wire:click="cancelDelete">
                        {{ $lang->data['no'] ?? 'No' }}
                    </button>
                    <button type="button" class="btn btn-danger text-md px-40 py-11 radius-8" wire:click="deleteOrder()" data-bs-dismiss="modal">
                        {{ $lang->data['yes'] ?? 'Yes' }}
                    </button>
                </div>
            </div>
