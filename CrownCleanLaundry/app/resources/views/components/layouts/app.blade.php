...
<body class="tw-text-sm">
    ...
    <script>
    // Tab/window close confirmation (only when actually leaving the app,
    // NOT on internal page navigation)
    (function () {
        // Mark internal link clicks so navigation within the app
        // doesn't trigger the confirmation dialog
        document.addEventListener('click', function (e) {
            var a = e.target && e.target.closest ? e.target.closest('a') : null;
            if (a && a.href && a.href.indexOf(location.origin) === 0) {
        ...
        window.addEventListener('beforeunload', function (e) {
            if (window.__internalNav) {
                window.__internalNav = false; // reset for next load
                return; // internal navigation — no confirmation
            }
            e.preventDefault();
            e.returnValue = '';
        });
