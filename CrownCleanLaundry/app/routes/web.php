...
Route::get('/license-activate', \App\Livewire\Auth\ActivateApp::class)->name('license.activate');
Route::get('/reset-password/{token}',\App\Livewire\Auth\ForgotPassword::class);

/*
|--------------------------------------------------------------------------
| Customer Self-Service Order Routes (Public - No Auth Required)
|--------------------------------------------------------------------------
| These routes allow customers to place orders via QR code scan
| Orders placed here will appear in the admin order list
*/
Route::group(['prefix' => 'order', 'middleware' => [\App\Http\Middleware\InstalledMiddleware::class]], function () {
    Route::get('/', \App\Livewire\CustomerOrder\PlaceOrder::class)->name('customer.order');
});
...
Route::group(['middleware' => [\App\Http\Middleware\InstalledMiddleware::class]], function () {
    ...
    Route::group(['prefix' => 'admin', 'middleware' => [Store::class]], function () {
        ...
        Route::group(['prefix' => 'settings/'], function () {
            ...
            Route::get('/print-settings', \App\Livewire\Settings\PrintSettings::class)->name('settings.print-settings');
